...
const GameplayScreen: React.FC<NavigationProps> = ({ onNavigate, activeWorld }) => {
  ...
  const handleSaveAndExit = async () => {
    ...
    // Updated data structure to match MainMenuScreen loading logic
    const saveData = {
        history: history,
        turnCount: turnCount,
        world: {
            player: activeWorld.player,
  ...
  const handleDownloadSave = () => {
    ...
    // Updated data structure to match MainMenuScreen import logic
    const saveData = {
        history: history,
        turnCount: turnCount,
        world: {
  ...
  // --- REUSABLE SIDEBAR CONTENT (For Mobile Drawer & Desktop Sidebar) ---
  const renderSidebarContent = (isMobile: boolean = false, onClose?: () => void) => (
      <div className="h-full flex flex-col bg-mystic-900 shadow-xl">
          {/* Header Section */}
          <div className="p-4 border-b border-slate-800 bg-mystic-800/50 shrink-0 flex flex-col gap-2">
              {isMobile && (
                  <div className="flex items-center justify-between mb-2 pb-2 border-b border-slate-700/50">
                      <h2 className="font-bold text-slate-200 flex items-center gap-2">
                          <Menu size={18} className="text-mystic-accent"/> Menu
                      </h2>
              ...
              <button onClick={() => { setShowCharModal(true); if(isMobile && onClose) onClose(); }} className="w-full flex items-center gap-3 p-3 bg-slate-800/50 hover:bg-slate-700 border border-slate-700 rounded-lg transition-all group">
              ...
              <button onClick={() => { setShowGlobalModal(true); if(isMobile && onClose) onClose(); }} className="w-full flex items-center gap-3 p-3 bg-slate-800/50 hover:bg-slate-700 border border-slate-700 rounded-lg transition-all group">
          ...
          <div className={`p-4 border-t border-slate-800 bg-mystic-900/95 space-y-2 mt-auto shrink-0 ${isMobile ? 'pb-8' : ''}`}>
            ...
            {showMobileSidebar && (
                <>
                    ...
                    <MotionDiv
                        ...
                        className="fixed top-0 left-0 bottom-0 z-[70] w-[85%] max-w-xs bg-mystic-900 border-r border-slate-800 md:hidden h-[100dvh] shadow-2xl"
                    >
                         {renderSidebarContent(true, () => setShowMobileSidebar(false))}
                    </MotionDiv>
                </>
            )}
        </AnimatePresence>
